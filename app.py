...
#     .main-header {
#         background: linear-gradient(90deg, #1a2744 0%, #0f1d35 100%);
#         border: 1px solid #2a4080;
#         border-radius: 16px;
#         padding: 2rem 2.5rem;
#         margin-bottom: 2rem;
#         box-shadow: 0 0 40px rgba(56, 130, 255, 0.1);
#     }
...
#     .churn-positive .prediction-value { color: #f87171; }
#     .churn-negative .prediction-value { color: #4ade80; }
...
#     return {
#         "tenure": {"type": "slider", "min": 0, "max": 72, "default": 12, "label": "Tenure (months)"},
#         "MonthlyCharges": {"type": "slider", "min": 18.0, "max": 120.0, "default": 65.0, "label": "Monthly Charges ($)", "step": 0.5},
#         "TotalCharges": {"type": "slider", "min": 18.0, "max": 9000.0, "default": 1500.0, "label": "Total Charges ($)", "step": 10.0},
#         "SeniorCitizen": {"type": "selectbox", "options": [0, 1], "default": 0, "label": "Senior Citizen"},
#         "gender": {"type": "selectbox", "options": ["Female", "Male"], "default": "Female", "label": "Gender"},
#         "Partner": {"type": "selectbox", "options": ["Yes", "No"], "default": "No", "label": "Has Partner"},
#         "Dependents": {"type": "selectbox", "options": ["Yes", "No"], "default": "No", "label": "Has Dependents"},
#         "PhoneService": {"type": "selectbox", "options": ["Yes", "No"], "default": "Yes", "label": "Phone Service"},
#         "MultipleLines": {"type": "selectbox", "options": ["Yes", "No", "No phone service"], "default": "No", "label": "Multiple Lines"},
#         "InternetService": {"type": "selectbox", "options": ["DSL", "Fiber optic", "No"], "default": "DSL", "label": "Internet Service"},
#         "OnlineSecurity": {"type": "selectbox", "options": ["Yes", "No", "No internet service"], "default": "No", "label": "Online Security"},
#         "OnlineBackup": {"type": "selectbox", "options": ["Yes", "No", "No internet service"], "default": "No", "label": "Online Backup"},
#         "DeviceProtection": {"type": "selectbox", "options": ["Yes", "No", "No internet service"], "default": "No", "label": "Device Protection"},
#         "TechSupport": {"type": "selectbox", "options": ["Yes", "No", "No internet service"], "default": "No", "label": "Tech Support"},
#         "StreamingTV": {"type": "selectbox", "options": ["Yes", "No", "No internet service"], "default": "No", "label": "Streaming TV"},
#         "StreamingMovies": {"type": "selectbox", "options": ["Yes", "No", "No internet service"], "default": "No", "label": "Streaming Movies"},
#         "Contract": {"type": "selectbox", "options": ["Month-to-month", "One year", "Two year"], "default": "Month-to-month", "label": "Contract Type"},
#         "PaperlessBilling": {"type": "selectbox", "options": ["Yes", "No"], "default": "Yes", "label": "Paperless Billing"},
#         "PaymentMethod": {"type": "selectbox", "options": ["Electronic check", "Mailed check", "Bank transfer (automatic)", "Credit card (automatic)"], "default": "Electronic check", "label": "Payment Method"},
#     }


# def build_sidebar_inputs(feature_names, encoders):
#     column_metadata = get_column_metadata()
#     user_inputs = {}

#     st.sidebar.markdown("""
#     <div style='padding: 1rem 0 0.5rem 0;'>
#         <span style='font-size: 1.3rem; font-weight: 700; color: #60a5fa;'>📡 Customer Profile</span>
#         <p style='color: #64748b; font-size: 0.8rem; margin-top: 0.3rem;'>Configure customer attributes below</p>
#     </div>
#     """, unsafe_allow_html=True)
...
#     for section_title, section_cols in section_groups.items():
#         visible_cols = [col for col in section_cols if col in feature_names]
#         if not visible_cols:
#             continue

#         st.sidebar.markdown(f"<div class='section-header'>{section_title}</div>", unsafe_allow_html=True)

#         for col in visible_cols:
#             meta = column_metadata.get(col, None)
#             if meta is None:
#                 if col in encoders:
#                     options = list(encoders[col].classes_)
#                     raw_value = st.sidebar.selectbox(col, options=options, key=col)
#                     user_inputs[col] = raw_value
#                 else:
#                     raw_value = st.sidebar.number_input(col, value=0.0, key=col)
#                     user_inputs[col] = raw_value
#             elif meta["type"] == "slider":
#                 step = meta.get("step", 1)
#                 raw_value = st.sidebar.slider(
#                     meta["label"], min_value=meta["min"], max_value=meta["max"],
#                     value=meta["default"], step=step, key=col
#                 )
#                 user_inputs[col] = raw_value
#             elif meta["type"] == "selectbox":
#                 raw_value = st.sidebar.selectbox(
#                     meta["label"], options=meta["options"],
#                     index=meta["options"].index(meta["default"]) if meta["default"] in meta["options"] else 0,
#                     key=col
#                 )
#                 user_inputs[col] = raw_value
...
# def encode_user_inputs(user_inputs, feature_names, encoders):
#     encoded_values = []
#     for feature in feature_names:
#         raw_value = user_inputs.get(feature, 0)
#         if feature in encoders and not isinstance(raw_value, (int, float)):
#             try:
#                 encoded_value = encoders[feature].transform([raw_value])[0]
#             except ValueError:
#                 encoded_value = 0
#         else:
...
# def render_prediction_result(prediction, churn_probability, no_churn_probability):
#     is_churn = prediction[0] == 1
#     card_class = "churn-positive" if is_churn else "churn-negative"
#     label_text = "⚠️ LIKELY TO CHURN" if is_churn else "✅ LIKELY TO STAY"
#     confidence = churn_probability if is_churn else no_churn_probability

#     st.markdown(f"""
#     <div class="prediction-card {card_class}">
#         <div class="prediction-label">Prediction Result</div>
#         <div class="prediction-value">{label_text}</div>
...
#             <div class="metric-value" style="color: #f87171;">{churn_probability:.1%}</div>
...
#         st.markdown(f"""
#         <div class="metric-card">
#             <div class="metric-value" style="color: #4ade80;">{no_churn_probability:.1%}</div>
#             <div class="metric-label">Retention Probability</div>
#         </div>
#         """, unsafe_allow_html=True)

#     st.markdown("<br>", unsafe_allow_html=True)
#     st.markdown(f"**Churn Risk Level**")
...
#         with col2:
#             st.markdown(f"<span style='font-family: JetBrains Mono; color: #60a5fa; font-size: 0.85rem;'>{row['Feature']}</span>", unsafe_allow_html=True)
...
#         <p>Real-time customer churn prediction powered by Random Forest · Configure customer attributes in the sidebar</p>
#     </div>
#     """, unsafe_allow_html=True)
...
#         st.error("⚠️ Model artifacts not found. Please run `python src/train.py` first to train the model.")
#         st.stop()

#     user_inputs = build_sidebar_inputs(feature_names, encoders)
...
#         if predict_clicked:
#             encoded_input = encode_user_inputs(user_inputs, feature_names, encoders)
#             prediction = model.predict(encoded_input)
#             probabilities = model.predict_proba(encoded_input)[0]
#             churn_probability = probabilities[1]
#             no_churn_probability = probabilities[0]
#             render_prediction_result(prediction, churn_probability, no_churn_probability)
#         else:
#             st.markdown("""
#             <div style='background: #0f1d35; border: 1px dashed #2a4080; border-radius: 12px; padding: 3rem; text-align: center;'>
#                 <div style='font-size: 3rem;'>🔮</div>
#                 <div style='color: #64748b; margin-top: 1rem; font-size: 0.95rem;'>
#                     Configure customer attributes in the sidebar<br>and click <strong style="color: #60a5fa;">Predict Churn</strong> to get a result.
#                 </div>
#             </div>
#             """, unsafe_allow_html=True)

#         st.markdown("<br>", unsafe_allow_html=True)
#         st.markdown("### 📋 Active Input Summary")
#         summary_data = []
#         for feature in feature_names:
#             val = user_inputs.get(feature, "N/A")
#             summary_data.append({"Feature": feature, "Value": str(val)})
#         st.dataframe(
...
#         st.markdown("<br>", unsafe_allow_html=True)
#         st.markdown("### 🧩 Loaded Features")
#         badges_html = "".join([f"<span class='feature-badge'>{f}</span>" for f in feature_names])
...
def build_sidebar(column_info, raw_columns):
    ...
    <div style='padding: 1rem 0 0.5rem;'>
        <span style='color: #f5f2ed; font-size: 1.3rem; font-weight: 800;'>🔭 ChurnIQ</span>
        <p style='color: #8892b0; font-size: 0.75rem; font-family: DM Mono, monospace; margin-top: 0.3rem;'>Customer Intelligence Platform</p>
    </div>
    """, unsafe_allow_html=True)
